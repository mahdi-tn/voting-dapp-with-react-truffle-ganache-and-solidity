...
class VerifyVoter extends Component {
  ...
  render() {
    ...
    if (this.state.voterList) {
      voterList = this.state.voterList.map((voter, index) => {
        return (
          <Card key={index} className="text-center mx-auto " bg='dark' text='light' style={{ width: '80%', marginTop: '20px' }}>
            ...
            <Card.Body className='bg-secondary' style={{ fontSize: "16px" }}>
              <Card.Text>
                ...
                <span className='text-dark'>{voter.name}</span>
              </Card.Text>
              <Card.Text>
                <b className='text-dark'>Ethereum Address: </b>
                <span className='text-dark'>{voter.voterAdd}</span>
              </Card.Text>
              <br />
